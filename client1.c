...
//	else printf("Client-connect() is OK...\n\n");
	...
	/* send IP & Port number */
	send(sockfd, USER1_IP, sizeof(USER1_IP)+1, 0);
	send(sockfd, _USER1_PORT, sizeof(_USER1_PORT)+1, 0);
		...
		if (pid == 0) {
			...
			while (1) {
				...
				if (!strcmp(msg, "[QUIT]")) {
					...
					my_addr.sin_port = htons(USER1_PORT);
			...
			while (1) {
				memset(buf, 0, 512);
				rcv_byte = recv(sockfd, buf, sizeof(buf)+1, 0);
				if (!strcmp(buf, " >> [user2] [FILE]")) {
					printf(" >> [user2's file request]\n");

					// recv user2's IP & port
					memset(buf, 0, 512);
					rcv_byte = recv(sockfd, buf, sizeof(buf)+1, 0);
